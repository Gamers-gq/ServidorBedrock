#!/bin/bash
# script para detener Servidor Minecraft Bedrock por MarcusGamer
...
while [[ $CountdownTime -gt 0 ]]; do
  if [[ $CountdownTime -eq 1 ]]; then
    screen -Rd servername -X stuff "say §c► §eDeteniendo Servidor en 60 segundos...$(printf '\r')"
    echo "Deteniendo el servidor en 60 segundos..."
    sleep 30;
    screen -Rd servername -X stuff "say §c► §eDeteniendo Servidor en 30 segundos...$(printf '\r')"
    echo "Deteniendo el servidor en 30 segundos..."
    sleep 20;
    screen -Rd servername -X stuff "say §c► §eDeteniendo Servidor en 10 segundos...$(printf '\r')"
    echo "Deteniendo el servidor en 10 segundos..."
    sleep 10;
    CountdownTime=$((CountdownTime-1))
  else
    screen -Rd servername -X stuff "say §c► §eDeteniendo Servidor en $CountdownTime minutos...$(printf '\r')"
...
screen -Rd servername -X stuff "say §c► §eDeteniendo Servidor§r (stop.sh called)...$(printf '\r')"
